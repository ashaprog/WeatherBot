...
def text(message):
    ...
    bot.send_message(message.chat.id, "Привет! Чтобы узнать погоду и рекомендацию по одежде от ChatGPT - нажми на кнопку и передай мне свое местоположение или нажми на кнопку 'Погода☀' чтобы ввести название города вручную", reply_markup=keyboard)


@bot.message_handler(func=lambda message: message.text == 'Помощь🆘')
def help(message):
    if message.text == 'Помощь🆘':
        bot.send_message(message.chat.id, 'Список команд:\n-Кнопка "Погода☀" отправит вашу геолокацию и бот расскажет вам о погоде в вашем городе, и сам ChatGPT подскажет вам что надеть)\n'
